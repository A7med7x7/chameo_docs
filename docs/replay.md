...
When chameo users complete their experiments, all the critical information gets automatically collected and stored through an MLflow server. This includes:
...
Our setup scripts use `[rclone](https://rclone.org/)` to mount the public metrics container locally, while MLflow connects directly to the artifacts container via S3 protocol. This gives you full access to browse experiment history and download any artifacts you need.
...
We use EC2 credentials by default. generate yours using the [generate_credentials.ipynb](http://github.com/A7med7x7/chameo/blob/mlflow-replay/notebooks/generate_credentials.ipynb) notebook. to access Chameleon Cloud expriement
